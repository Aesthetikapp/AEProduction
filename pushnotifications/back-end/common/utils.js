...
function getPatientPayment(patientpayment, obj) {
	const obj1 = obj;
	// const keys = Object.keys(patient["_doc"]);

	let u = iterate(patientpayment, obj1);
	if (keyExists("address", obj1)) {
		u = iteratePatient(patientpayment["address"][0], obj1["address"][0]);
	}
	if (keyExists("address1", obj1)) {
		u = iteratePatient(patientpayment["address1"][0], obj1["address1"][0]);
	}
	if (keyExists("address2", obj1)) {
		u = iteratePatient(patientpayment["address2"][0], obj1["address2"][0]);
	}
	if (keyExists("address3", obj1)) {
		u = iteratePatient(patientpayment["address3"][0], obj1["address3"][0]);
	}
	return patientpayment;
}

function getPatientPaymentDetails(patientpaymentdetails, obj) {
	const obj1 = obj;
	// const keys = Object.keys(patient["_doc"]);

	let u = iterate(patientpaymentdetails, obj1);
	if (keyExists("address", obj1)) {
		u = iteratePatient(patientpaymentdetails["address"][0], obj1["address"][0]);
	}
	if (keyExists("address1", obj1)) {
		u = iteratePatient(
			patientpaymentdetails["address1"][0],
			obj1["address1"][0]
		);
	}
	if (keyExists("address2", obj1)) {
		u = iteratePatient(
			patientpaymentdetails["address2"][0],
			obj1["address2"][0]
		);
	}
	if (keyExists("address3", obj1)) {
		u = iteratePatient(
			patientpaymentdetails["address3"][0],
			obj1["address3"][0]
		);
	}
