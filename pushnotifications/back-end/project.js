...
function Airshippushnotifications() {
	...
	appointment.aggregate(
		...
		function (err, result) {
			// console.log(
			// 	"result of appt",
			// 	result[0].settings[0].notification[0].e_apt_activity,
			// 	result[0].settings[0].notification[0].e_conslt_request
			// );
			// console.log("result", result.length);
			if (result !== undefined && result.length !== 0) {
				// console.log(
				// 	"enter",
				// 	result[0].patient_details[0].channelid.replaceAll("|", ",")
				// );
				// console.log("result of appt", result[0].doctor_details[0].firstName);
				var appointmenttime = moment(result[0].starttime, "HH:mm");
				var consultationtime = moment(result[0].cstarttime, "HH:mm");
				// console.log("appointmenttime", appointmenttime);
				// console.log(
				// 	"time",
				// 	appointmenttime.diff(currentTime, "minutes"),
				// 	consultationtime.diff(currentTime, "minutes")
				// 	// moment.duration((result[0].starttime).diff(currentTime))
				// );
				// console.log("result of appt", result[0].appointmentdate);
				// console.log(
				// 	moment(result[0].appointmentdate).format("YYYY-MM-DD") +
				// 		"T00:00:00.000+00:00" ===
				// 		currentdate
				// );
				var Notifications = [
					{
						expression:
							moment(result[0].appointmentdate).format("YYYY-MM-DD") +
								"T00:00:00.000+00:00" ===
								nextdate && appointmenttime.diff(currentTime, "minutes") === 0,
						patienttitle: "Appointment Reminder",
						patientvalue:
							"Hey " +
							result[0].patient_details[0].firstName +
							" your appointment with " +
							result[0].doctor_details[0].firstName +
					...
					{
						expression:
							moment(result[0].consultationdate).format("YYYY-MM-DD") +
								"T00:00:00.000+00:00" ===
								nextdate && consultationtime.diff(currentTime, "minutes") === 0,
					...
					if (noti.expression === true) {
						...
						result.forEach((patient) => {
							patientsettings.find(
								...
								function (error, res) {
									let patientData = returnNotificationData(
										result[0].patient_details[0].channelid.split("|"),
										noti.patientvalue,
										noti.patienttitle,
										"patient"
									);
							...
							if (doctorcheck === true) {
								let doctorData = returnNotificationData(
									result[0].doctor_details[0].channelid.split("|"),
									noti.doctorvalue,
									noti.doctortitle,
									"doctor"
								);
				...
				if (
					...
					result.forEach((patient) => {
						if (
							result[0].ispatientjoined === false &&
							result[0].isdoctorjoined === true
						) {
							patientsettings.find(
								{
									patientid: patient.patientid,
							...
							if (
								result[0].settings[0].notification[0].e_conslt_request === true
							) {
								let doctorData = returnNotificationData(
									result[0].doctor_details[0].channelid.split("|"),
									"Hey " +
							...
							result[0].ispatientjoined === true &&
							result[0].isdoctorjoined === false
						) {
							patientsettings.find(
								{
									patientid: patient.patientid,
