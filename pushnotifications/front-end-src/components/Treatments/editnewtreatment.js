...
// import { treatmentlist } from "../../services/notificationservices";
// import * as treatmentlist from "../../services/treatments";
...
const EditNewTreatment = (props) => {
	console.log("new props", props);
	const firstupdate = useRef(true);
	const [treatments] = useState([]);
	const [team, setTeam] = useState([]);
	const [treatmentData, setTreatmentData] = useState(undefined);
	...
	useLayoutEffect(() => {
		if (firstupdate.current) {
			// TreatmentService.GetGlobalTreatments().then((result) => {
			// 	result.globaltreatments.forEach(function (elem) {
			// 		if (			!treatments.includes(
			// 				elem.treatmenttype + elem.bodyarea + elem.treatmentname
			// 			)
			// 		) {
			// 			treatments.push(
			// 				elem.treatmenttype + elem.bodyarea + elem.treatmentname
			...
			// 	});
			// });
			// console.log("combinedList", treatments);
			let clinic = props.clinicname;
			// let clinic = "Hyderabad";
			...
			UserServices.getDoctors(clinic).then(function (result) {
				...
				// if (result.data.deleteImage.id) {
				//   window.location.reload();
				// }
			});

			TreatmentServices.GetTreatmentById(props.id).then(function (result) {
				console.log("edittreatment", result.treatmentsByID);
				setTreatmentData(result.treatmentsByID);
			});
		}
	}, []);
	...
	useEffect(() => {
		(async function anyNameFunction() {
			...
			// console.log("def_treatmenttype", def_treatmenttype);
	...
	// console.log(
	// 	defaultpop,
	// 	owndisclosure.Disclosuretitle + "|" + owndisclosure.Body
	// );
	const confirm = (value) => {
		...
		setPrevstatevalues();
		// console.log("ff", treatments[0]);
		// console.log("dd", treatmentType + bodyArea + treatmentName);
		if (treatments.includes(treatmentType + bodyArea + treatmentName)) {
			alert("No duplicate treatments allowed");
			return false;
		}
		// if (value === "Disabled") {
		// 	console.log("disavle enter");
		// 	props.save(false);
		// }
		...
		if (
			...
			// if (treatmentType !== "") {
			// 	data.append("treatmentName", treatmentName);
			// 	data.append("treatmentType", treatmentType);
			// 	data.append("bodyArea", bodyArea);
			// } else {
			// 	data.append("treatmentName", tname);
			// 	data.append("treatmentType", ttype);
			// 	data.append("bodyArea", barea);
			// }
			data.append(
				"treatmentName",
			...
			// data.append("disclosure", disclosure === "Templete Added" ? defaultpop: owndisclosure);
			// var doctors = [];
			// assignDoctor.forEach(function (entity) {
			// 	setDoctors((prevState) => [
			// 		...prevState,
			// 		{ userid: entity.id, avatar: entity.avatar },
			// 	]);
			// 	// doctors.push(entity.id);
			// });
			// setAssignDoctor((prevState) => [
			// 	prevState.filter((k) => {
			// 		return k.id;
			// 	}),
			// ]);
			console.log("assignDoctor", assignDoctor);
			// let obj = { userid: "", avatar: "" };
			let arrobj = [];
			assignDoctor.forEach((entity) => {
				// console.log("enter docotoes", entity.id, entity.avatar);
			...
			// data.append("treatment type", ttype);
			// data.append("body area", barea);
			// data.append("treatment name", tname);
			...
			// if (event === "Use Templete") {
			// 	setDisclosure("Templete Added");
			// data.append("disclosurepop", owndisclosure);
			// data.append("defaultpop", defaultpop);
			...
			// data.append("video", photo1 !== "" ? video.name : "");
			var object = {};
			...
			// 	...prevState,
			// 	name:
			// 		object["treatmentType"].replace(/\s/gi, "") +
			// 		"_" +
			// 		object["bodyArea"].replace(/\s/gi, "") +
			// 		"_" +
			// 		object["treatmentName"].replace(/\s/gi, "") +
			// 		"_" +
			// 		"video.mp4",
			// }));
			data1.append(
			...
			//   setValidClassName("notvalid");
			// }
			// var json = JSON.stringify(object);
			const updatestr = TreatmentService.returnCreateVariables(object);
			console.log(updatestr);
			//data1.forEach((value, key) => { if (value != "") { console.log(value) } });
			...
			if (value === "Disabled") {
				...
				props.change(true);
				// }, 5000);
				// setTimeout(props.change(true), 5000);
			} else {
				window.location.reload();
			}
		} else {
	...
	const disableTreatmentHandler = (id) => {
		const data = new FormData();
		data.append("id", id);
		data.append("description", textarea);
		var object = {};
		...
		var treatmenttype = treatmentData.treatmenttype;
		var bodyarea = treatmentData.bodyarea;
		var treatmentname = treatmentData.treatmentname;
			...
			if (value !== "") {
				...
				if (key === "photo1" || key === "photo2" || key === "video") {
					object[key] = object[key]
						.replace(
							"#treatmenttype#",
							treatmenttype.replace(/[^A-Za-z0-9:._-]/gi, "")
						)
						.replace("#bodyarea#", bodyarea.replace(/[^A-Za-z0-9:._-]/gi, ""))
						.replace(
							"#treatmentname#",
							treatmentname.replace(/[^A-Za-z0-9:._-]/gi, "")
						)
						.replace(/[^A-Za-z0-9:/._-]/gi, "");
				}
			}
		...
		data1.append(
			...
			new File(
				...
				treatmenttype.replace(/[^A-Za-z0-9:._-]/gi, "") +
					"_" +
					bodyarea.replace(/[^A-Za-z0-9:._-]/gi, "") +
					"_" +
					treatmentname.replace(/[^A-Za-z0-9:._-]/gi, "") +
					"_" +
					"photo1.png",
	...
	const onImageChange = (image) => {
		...
		//image = new File([image], "photo1.png", { type: "image/png" });
		console.log("1", image);
		//images.append("File1", image);
		setphoto1(image);
	};
	const onImageChange2 = (image) => {
		// image.name = "Profile.png";
		//image = new File([image], "photo2.png", { type: "image/png" });
		console.log("2", image);
		//images.append("File2", image);
		setphoto2(image);
	};
			...
			<div>
				...
				<div className="row" style={{ padding: "20px" }}>
					...
					<div className="col-xl col-lg col-md col-xs col-sm">
						...
						<div className="row  pt-3 pb-4" style={{ pointerEvents: "none" }}>
							<div
									...
									{click ? (
										<>
											<Customdropdown
												...
												ttype="edit"
												ttypetext={treatmentData && treatmentData.treatmenttype}
											></Customdropdown>
										</>
									) : (
										<>
									...
									{clicktwo ? (
										<>
											<Customdropdowntwo
												...
												ttype="edit"
												ttypetext={treatmentData && treatmentData.bodyarea}
											></Customdropdowntwo>
										</>
									) : (
										<>
									...
									{clickthree ? (
										<>
											<Customdropdownthree
												...
												ttype="edit"
												ttypetext={treatmentData && treatmentData.treatmentname}
											></Customdropdownthree>
										</>
									) : (
										<>
						...
						<div className="row  pt-3 pb-4">
							...
							>
								<textarea
									...
									maxLength="300"
									defaultValue={treatmentData && treatmentData.description}
									style={{
										borderRadius: "10px",
									}}
						...
						<div className="row  pt-3 pb-4">
							...
							>
								<input
									...
									defaultValue={
										treatmentData && treatmentData.quantityavailable
									}
								...
								<div className="row pt-3 pb-4">
									<div className="col-xl col-lg col-md col-xs col-sm ">
										<input
											...
											disabled
											defaultValue={
												treatmentData && treatmentData.pricepersyring
											}
									...
									<div className="col-xl col-lg col-md col-xs col-sm ">
										<input
											...
											disabled
											defaultValue={treatmentData && treatmentData.sellingprice}
							...
							<div className="row  pt-3 pb-4">
								...
								>
									<input
										...
										defaultValue={treatmentData && treatmentData.duration}
									...
									<div className="row">
										<div className="col-xl-3 col-lg-3 col-md-3 col-xs-3 col-sm-3">
											<select
												...
												defaultValue={
													treatmentData && treatmentData.advancedsetting
												}
												disabled
					...
					<div className="col-xl col-lg col-md col-xs col-sm">
							...
							<div className="row">
								...
								>
									<Dragcropmodal
										...
										border1={valid}
										ttype="edit"
										ttypetext={treatmentData && treatmentData.photo1}
									></Dragcropmodal>
								</div>
							</div>
						</div>
						<div className="row  pt-3 pb-4">
							...
							<div className="row">
								...
								<div
									className="col-xl-4 col-lg-4 col-md-4 col-xs-4 col-sm-4 "
									style={{ paddingLeft: "0px", pointerEvents: "none" }}
								>
									<Cropmodal2
							...
							>
								<>
									<Customdropdownfour
										...
										doctors={team}
										ttype="edit"
										ttypetext={treatmentData && treatmentData.assigneddoctors}
									></Customdropdownfour>
								</>
							</div>
						</div>
                    ...
                    Save and Exit
							...
							<div className="col-xl col-lg col-md col-xs col-sm">
								<button
									...
									//   onClick={confirm}
									onClick={() => {
										disableTreatmentHandler(treatmentData.id);
									}}
								>
									{" "}
									<label
...
export default EditNewTreatment;
